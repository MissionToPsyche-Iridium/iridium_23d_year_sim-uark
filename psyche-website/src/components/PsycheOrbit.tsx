...
function Scene({ showOrbits, setShowOrbits, setFocusPsyche, focusPsyche, selectedOrbit, setSelectedOrbit }: any) {
  ...
  return (
    <>
      ...
      {/* Psyche Centered View */}
      {focusPsyche && (
        <>
          <mesh position={[0, 0, 0]} onClick={() => { setShowOrbits(false); setFocusPsyche(false); setSelectedOrbit(null); }}>
          ...
          {showOrbits && ORBITS.map((orbit) => (
            <group key={orbit.name} rotation={orbit.rotation}>
              <Line
                points={new THREE.EllipseCurve(0, 0, orbit.radiusX, orbit.radiusZ, 0, 2 * Math.PI, false, 0).getPoints(100).map((p) => new THREE.Vector3(p.x, 0, p.y))}
                color={orbit.color}
                lineWidth={2}
                dashed={false}
              />
              <mesh
                ...
                position={[0, 0, 0]}
              >
                <ringGeometry args={[orbit.radiusX - 0.05, orbit.radiusX + 0.05, 64]} />
